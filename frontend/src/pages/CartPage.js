...
const CartPage = () => {
    ...
    const getCart = async () => {
        try {
            const { data } = await axios.get(`https://e-commerce-9m1c.vercel.app/api/cart/getcart/${auth.user._id}`);
            const productIds = data.cart[0]?.product || [];

            const productDetails = await Promise.all(
                productIds.map((id) => axios.get(`https://e-commerce-9m1c.vercel.app/api/product/get-single-product/${id}`))
            );
    ...
    const removeCartItem = async (id) => {
        try {
            await axios.delete(`https://e-commerce-9m1c.vercel.app/api/cart/removeproduct/${auth.user._id}`, { data: { product: id } });
            getCart();
        } catch (error) {
            console.log(error);
        }
